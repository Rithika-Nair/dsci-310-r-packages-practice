...
A fairly common task when dealing with numerical values is to sum them up. `regexcite::my_add()` adds up two numeric parameter inputted by the user. If only one numeric parameter is provided, the function will add it up with 10. If either of the parameters are NA, `my_add()` will return NA. If either of the parameters are strings, an error will be thrown.
